...
test('Query Case', async ({ page }) => {
    await page.goto('https://peopleservices--wowpsuat.sandbox.my.salesforce.com/');
    await page.locator('id=username').fill('akm@tcs.woolworths.com.au.hrprod.wowpsuat')
    await page.locator('id=password').fill('Happy@1234');
    await page.getByRole('button', { name: 'Log In to Sandbox' }).click();
    await page.getByRole('button', { name: 'Setup' }).waitFor({ state: "visible" });
    await page.getByRole('button', { name: 'Setup' }).click();
    ...
    await page1.getByPlaceholder('Search Setup').fill('Thomas Padget');
    await page1.getByRole('option', { name: 'Thomas Padget User' }).click();
    await page1.locator('iframe').contentFrame().getByRole('row', { name: 'User Detail Edit Sharing' }).locator('input[name="login"]').click();
    await page1.getByRole('button', { name: 'App Launcher' }).waitFor({ state: "visible" });
    await page1.waitForTimeout(3000);
    await page1.getByRole('button', { name: 'App Launcher' }).click();
    await page1.waitForTimeout(3000);
    await page1.getByPlaceholder('Search apps and items...').type('PeopleServices AI');
    await page1.waitForTimeout(10000);
    await page1.getByRole('option', { name: 'PeopleServices AI' }).waitFor({ state: "visible" });
    await page1.getByRole('option', { name: 'PeopleServices AI' }).click()


    const [page2] = await Promise.all([
        page1.waitForEvent("popup"), await page1.getByRole('option', { name: 'PeopleServices AI' }).click()
    ]);
    await page2.locator('a').filter({ hasText: 'Raise a People Services' }).click();
    await page2.getByRole('heading', { name: 'Benefit' }).click();
    await page2.locator('label').filter({ hasText: 'No' }).locator('span').first().click();
